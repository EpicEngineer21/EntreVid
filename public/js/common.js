...
window.renderNav = function renderNav(user, activePage) {
  ...
  if (user) {
    ...
    if (role === 'admin') {
      desktopLinks += `
        <a href="/admin/applications" id="nav-admin" class="nav-link px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${activePage === 'admin' ? 'text-white bg-white/10' : 'text-purple-400 hover:text-purple-300 hover:bg-purple-500/10'}">
